...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        # forward the GPT modle itself
        tok_emb = self.transformer.wte(idx)  # token embeddings of shape (b, t, n_embd)
        pos_emb = self.transformer.wpe(
            pos
        )  # positional embeddings of shape (t, n_embd), there is no batch size here because the same positional encodings are used for each example in the batch
        ...
        if targets is not None:
            ...
            # inference time mini-optimization: only forward the lm_head on the last position
            logits = self.lm_head(
                x[:, [-1], :]
    ...
    def configure_optimizers(self, weight_decay, learning_rate, betas, device_type):
        ...
        fused_availabe = "fused" in inspect.signature(torch.optim.AdamW).parameters
        use_fused = device_type == "cuda" and fused_availabe
    ...
    def estimate_mfu(self, fwdbwd_per_iter, dt):
        """Estimate model flops utilization (MFU) in units of A100
        bfloat16 peak FLOPS."""
    ...
    def generate(self, idx, max_new_tokens, temperature=1.0, top_k=None):
        ...
        shape (b,t)) and complete the sequence max_new_tokens times,
        feeding the predictions back into the model each time.

        Most likely you'll want to make sure to be in model.eval() mode
        ...
        for _ in range(max_new_tokens):
            ...
            # append smapled index to the running sequence and continue
